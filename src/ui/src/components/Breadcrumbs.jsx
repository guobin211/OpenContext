...
export function Breadcrumbs({ selectedDoc, saveState }) {
  ...
  if (!selectedDoc?.rel_path) {
    return <span className="text-sm text-gray-400">{t('emptyState.title')}</span>;
  }
      ...
      {selectedDoc.rel_path.split('/').map((part, i, arr) => {
        ...
        return (
          ...
          <div key={i} className="flex items-center gap-2 whitespace-nowrap">
            ...
            <span className={`${isLast ? 'font-medium text-gray-900' : 'hover:text-gray-700 cursor-pointer'}`}>
              {part.replace('.md', '')}
            </span>
          </div>
        );
      })}
      {saveState === 'saving' && <ArrowPathIcon className="h-3 w-3 animate-spin text-gray-400 ml-2" />}
